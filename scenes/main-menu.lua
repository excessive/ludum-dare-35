...
local menu = tiny.system {
	name = "main menu",
	prev_scene = "scenes.splash"
}

function menu:enter(from)
	local ngp = love.filesystem.isFile("ngp")
	...
	self.options = {
		...
		default = 1,
		sfx = {
			prev = love.audio.newSource("assets/sfx/prev.wav"),
			next = love.audio.newSource("assets/sfx/next.wav"),
			pick = love.audio.newSource("assets/sfx/pick.wav")
		},
		{ text = "main/play",      action = function() Scene.switch(require "scenes.letter") end },
		{ text = "main/options",   action = function() Scene.switch(require "scenes.options") end },
		{ text = "main/credits",   action = function() Scene.switch(require "scenes.credits") end },
		{ text = "main/exit",      action = function() console.invokeCommand("quit") end }
	}

	if ngp then
		table.insert(self.options, 2, { text = "main/play+", action = function() Scene.switch(require "scenes.letter", true) end })
	end

	if FLAGS.debug_mode then
		-- table.insert(self.options, 4, { text = "main/debug", action = function() Scene.switch(require "scenes.love3d-test") end })
		-- table.insert(self.options, 5, { text = "main/crash", action = function() Scene.switch(require "scenes.crash-test") end })
	end
	...
	love.graphics.setBackgroundColor(80, 135, 95, 255)
...
function menu:prev_screen()
	assert(self.prev_scene)
	Scene.switch(require(self.prev_scene))
end
...
function menu:next_screen()
	...
	option.action()
	-- local ok, screen = pcall(option.action)
	-- if not ok then
...
local function check_hit(menu, x, y)
	for i, v in ipairs(menu.cache) do
		if cpml.intersect.point_aabb(cpml.vec3(x, y, 0), {
			min = cpml.vec3(v.x, v.y, -0.1),
			max = cpml.vec3(v.x + v.w, v.y + v.h,  0.1)
		}) then
			return i
		end
	end
...
function menu:update(dt)
	...
	local buttons = self.world.inputs.game
	if buttons.menu_up:pressed() then
		self:prev_option()
	end
	if buttons.menu_down:pressed() then
		self:next_option()
	end

	if buttons.menu_back:pressed() then
		self:transition_out(true)
	end
	if buttons.menu_action:pressed() then
		self:transition_out()
	end
end
...
function menu:draw()
	...
	for i, option in ipairs(self.options) do
		...
		love.graphics.print((self.world.language(option.text)), x, y - 2)
		love.graphics.setColor(255, 255, 255, 255)
		if i == self.options.current then
	...
	love.graphics.print(
		FLAGS.game_version,
		anchor:right() - font:getWidth(FLAGS.game_version),
		anchor:top()
	)
