...
function menu:enter(from)
	...
	self.time = 0
	self.clicked = false
	self.prev = require("scenes.main-menu")
	self.options = {
		cache   = {},
		default = 1,
		sfx = {
			...
			"options/language",
			function()
				love.audio.play(self.options.sfx.pick)
			...
			"options/volume_up",
			function()
				PREFERENCES.volume = cpml.utils.clamp(PREFERENCES.volume + 0.1, 0, 1)
	...
	convoke(function(continue, wait)
		...
		self.timer.add(0.75, function()
			self.locked = false
		end)
		self.timer.tween(0.5, self.state, { opacity = 0 }, 'out-quad')
		-- self.timer.add(10, continue())
		-- wait()
		-- self:transition_out()
	end)()
...
function menu:mousereleased(x, y, button)
	...
	if hit and hit == self.clicked then
		...
		self.options[self.options.current][2]()
	end
end
...
function menu:draw()
	...
	for i, option in ipairs(self.options) do
		...
		love.graphics.print((self.world.language(option[1])), x, y - 2)
		love.graphics.setColor(255, 255, 255, 255)
		if i == self.options.current then
	...
	if self.world.inputs.game.menu_up:pressed() then
		self:prev_option()
	end
	if self.world.inputs.game.menu_down:pressed() then
		self:next_option()
	end
